...
async function ensure(db){
 await db.prepare(`CREATE TABLE IF NOT EXISTS hubcore_users (id TEXT PRIMARY KEY,created_at INTEGER NOT NULL DEFAULT 0,name TEXT NOT NULL DEFAULT '',email TEXT NOT NULL DEFAULT '',username TEXT NOT NULL DEFAULT '',password_hash TEXT NOT NULL DEFAULT '',password_salt TEXT NOT NULL DEFAULT '',profile_photo TEXT NOT NULL DEFAULT '',terms_accepted_at INTEGER NOT NULL DEFAULT 0,terms_version TEXT NOT NULL DEFAULT '')`).run();
...
export async function onRequestPost({request,env}){if(!env?.DB)return json({error:'Account database unavailable.'},503);try{await ensure(env.DB);const b=await request.json(),action=clean(b.action,20);
 if(action==='signup'){
  ...
  const taken=await env.DB.prepare('SELECT id FROM hubcore_users WHERE lower(email)=? OR lower(username)=?').bind(email,username.toLowerCase()).first();if(taken)return json({error:'That email or username is already registered.'},409);
  const id=crypto.randomUUID(),salt=crypto.randomUUID(),ph=await hashPassword(password,salt),now=Date.now();
  await env.DB.prepare(`INSERT INTO hubcore_users(id,created_at,name,email,username,password_hash,password_salt,profile_photo,terms_accepted_at,terms_version) VALUES(?,?,?,?,?,?,?,?,?,?)`).bind(id,now,name,email,username,ph,salt,'',now,TERMS_VERSION).run();
  const token=crypto.randomUUID()+crypto.randomUUID(),th=await digest(token);await env.DB.prepare('INSERT INTO hubcore_sessions(token_hash,user_id,expires_at,created_at) VALUES(?,?,?,?)').bind(th,id,now+2592000000,now).run();
  let notificationSent=false;try{notificationSent=await notify(env,{name,email,username})}catch{}
  return json({ok:true,token,user:{id,name,email,username,profilePhoto:'',termsAcceptedAt:now,termsVersion:TERMS_VERSION},notificationSent},201)
 }
 if(action==='login'){const login=clean(b.login,200).replace(/^@/,'').toLowerCase(),password=String(b.password||''),u=await env.DB.prepare('SELECT * FROM hubcore_users WHERE lower(email)=? OR lower(username)=?').bind(login,login).first();if(!u||!u.password_salt||!u.password_hash||await hashPassword(password,u.password_salt)!==u.password_hash)return json({error:'Incorrect email, username or password.'},401);const token=crypto.randomUUID()+crypto.randomUUID(),th=await digest(token),now=Date.now();await env.DB.prepare('INSERT INTO hubcore_sessions(token_hash,user_id,expires_at,created_at) VALUES(?,?,?,?)').bind(th,u.id,now+2592000000,now).run();return json({ok:true,token,user:{id:u.id,name:u.name,email:u.email,username:u.username,profilePhoto:u.profile_photo||'',termsAcceptedAt:Number(u.terms_accepted_at||0),termsVersion:u.terms_version||''}})}
...
}catch(e){console.error('HubCore account POST error',e);return json({error:'Unable to process account request. Please try again in a moment.'},500)}}