...
function capabilities(env){return {community:true,profilePhotos:true,reelFeed:true,reelUploads:Boolean(env?.MEDIA),emailNotifications:hubcoreEmailStatus(env).emailReady}}
...
async function ensure(db){
 await db.prepare(`CREATE TABLE IF NOT EXISTS hubcore_users (id TEXT PRIMARY KEY,created_at INTEGER NOT NULL DEFAULT 0,name TEXT NOT NULL DEFAULT '',email TEXT NOT NULL DEFAULT '',username TEXT NOT NULL DEFAULT '',password_hash TEXT NOT NULL DEFAULT '',password_salt TEXT NOT NULL DEFAULT '',password_iterations INTEGER NOT NULL DEFAULT 150000,profile_photo TEXT NOT NULL DEFAULT '',terms_accepted_at INTEGER NOT NULL DEFAULT 0,terms_version TEXT NOT NULL DEFAULT '')`).run();
...
export async function onRequestGet({request,env}){if(!env?.DB)return json({error:'Account database unavailable.'},503);try{await ensure(env.DB);const u=await sessionUser(request,env.DB);return u?json({user:{id:u.id,name:u.name,email:u.email,username:u.username,profilePhoto:u.profile_photo||'',termsAcceptedAt:Number(u.terms_accepted_at||0),termsVersion:u.terms_version||''},capabilities:capabilities(env)}):json({error:'Please log in.'},401)}catch(e){console.error('HubCore account GET error',e);return json({error:'Unable to load account.'},500)}}
