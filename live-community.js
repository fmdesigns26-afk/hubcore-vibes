(() => {
  ...
  const editKeyStore='hubcore-community-edit-keys-v1';
  ...
  function editKeys(){return readJSON(editKeyStore,{});}
  function editKeyFor(postId){return String(editKeys()[postId]||'');}
  function saveEditKey(postId,key){const keys=editKeys();keys[postId]=key;saveJSON(editKeyStore,keys);}
  function makeEditKey(){return crypto.randomUUID?.()||`${Date.now()}-${Math.random().toString(36).slice(2)}-${Math.random().toString(36).slice(2)}`;}
  async function registerEditKey(postId,editKey){
    const r=await fetch('/api/community-edit',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'register',postId,editKey})});
    const d=await r.json().catch(()=>({}));if(!r.ok)throw new Error(d.error||'Unable to enable editing');return d;
  }
  async function updatePost(postId,text,editKey){
    const r=await fetch('/api/community-edit',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'edit',postId,text,editKey})});
    const d=await r.json().catch(()=>({}));if(!r.ok)throw new Error(d.error||'Unable to edit post');return d;
  }
  ...
  function render(){
    ...
    feed.innerHTML=posts.length?posts.map(post=>{
      const mine=selected[post.id]||null,counts=post.reactions||{},commentCount=(post.comments||[]).length,shareCount=Number(post.shares||0),canEdit=Boolean(editKeyFor(post.id));
      return `<article class="post-card glass-panel" id="community-post-${esc(post.id)}" data-post-id="${esc(post.id)}">
        ...
        ${canEdit?`<form class="post-edit-form" data-post-id="${esc(post.id)}" hidden><textarea name="postEditText" rows="5" maxlength="50000" required>${esc(post.text)}</textarea><div class="post-edit-actions"><button class="btn primary post-edit-save" type="submit">Save changes</button><button class="post-action post-edit-cancel" type="button" data-post-id="${esc(post.id)}">Cancel</button><span class="post-edit-status" role="status" aria-live="polite"></span></div></form>`:''}
  ...
  async function refreshMetrics(){
    try{
      ...
      document.querySelectorAll('[data-live-reach]').forEach(el=>{el.textContent=Number(reach[el.dataset.liveReach]||0).toLocaleString();});
    }catch(error){console.warn('Unable to refresh community metrics:',error);}
  }
  ...
  document.addEventListener('click',async event=>{
    ...
    const editButton=event.target.closest('.edit-post-btn');if(editButton){event.preventDefault();event.stopImmediatePropagation();const card=document.querySelector(`.post-card[data-post-id="${editButton.dataset.postId}"]`),text=card?.querySelector('.post-text'),form=card?.querySelector('.post-edit-form');if(text&&form){text.hidden=true;form.hidden=false;form.querySelector('textarea')?.focus();}return;}
    const editCancel=event.target.closest('.post-edit-cancel');if(editCancel){event.preventDefault();event.stopImmediatePropagation();const card=document.querySelector(`.post-card[data-post-id="${editCancel.dataset.postId}"]`),text=card?.querySelector('.post-text'),form=card?.querySelector('.post-edit-form');if(text&&form){form.hidden=true;text.hidden=false;const original=posts.find(p=>p.id===editCancel.dataset.postId)?.text||'';const box=form.querySelector('textarea');if(box)box.value=original;}return;}
  ...
  document.addEventListener('submit',async event=>{
    ...
    if(editForm){event.preventDefault();event.stopImmediatePropagation();const postId=editForm.dataset.postId,key=editKeyFor(postId),textarea=editForm.querySelector('[name="postEditText"]'),button=editForm.querySelector('.post-edit-save'),status=editForm.querySelector('.post-edit-status'),text=textarea?.value.trim();if(!text){if(status)status.textContent='Post text cannot be empty.';return;}if(!key){if(status)status.textContent='This post cannot be edited from this device.';return;}if(button)button.disabled=true;if(status)status.textContent='Saving changes…';try{await updatePost(postId,text,key);await refresh();}catch(error){if(status)status.textContent=error?.message||'Unable to save changes.';}finally{if(button)button.disabled=false;}return;}
    const commentForm=event.target.closest('.comment-form');
    if(commentForm){event.preventDefault();event.stopImmediatePropagation();const textarea=commentForm.querySelector('textarea'),button=commentForm.querySelector('.comment-send'),status=commentForm.querySelector('.comment-status'),founder=founderSession();let name='Yutani Pretorius',handle='@yutanipretorius';if(!founder){name=commentForm.querySelector('[name="commentName"]')?.value.trim();handle=normalizeHandle(commentForm.querySelector('[name="commentUsername"]')?.value);if(!name||!handle){if(status)status.textContent='Please enter your name and username.';return;}saveProfile(name,handle);}const text=textarea.value.trim();if(!text){if(status)status.textContent='Please write your comment.';return;}const comment={id:`comment-${crypto.randomUUID?.()||Date.now()}`,author:founder?'Founder · Yutani Pretorius · @yutanipretorius':`${name} · ${handle}`,text,timestamp:Date.now(),replyTo:textarea.dataset.replyTo||null};if(button)button.disabled=true;if(status)status.textContent='Posting comment…';try{await API().createComment(commentForm.dataset.postId,comment);textarea.value='';delete textarea.dataset.replyTo;textarea.placeholder='Write your comment — long comments are welcome…';await refresh();await refreshMetrics();}catch(error){console.error('Unable to post comment:',error);if(status)status.textContent=error?.message||'Comment could not be posted. Please try again.';}finally{if(button)button.disabled=false;}return;}
    const composer=event.target.closest('#communityForm');
    if(composer){event.preventDefault();event.stopImmediatePropagation();const input=document.getElementById('composerInput'),founder=founderSession();let name='Yutani Pretorius',handle='@yutanipretorius';if(!founder){name=composer.querySelector('[name="displayName"]')?.value.trim();handle=normalizeHandle(composer.querySelector('[name="username"]')?.value);if(!name||!handle)return;saveProfile(name,handle);}const text=input?.value.trim();if(!text)return;const post={id:`post-${crypto.randomUUID?.()||Date.now()}`,name,handle,avatar:initials(name),timestamp:Date.now(),text,reactions:{like:0,hub:0,fire:0,inspire:0},comments:[],shares:0},editKey=makeEditKey();await API().createPost(post);try{await registerEditKey(post.id,editKey);saveEditKey(post.id,editKey);}catch(error){console.warn('Post created, but editing could not be enabled:',error);}input.value='';const count=document.getElementById('composerCount');if(count)count.textContent='280';await refresh();return;}
  },true);
